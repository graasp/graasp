...
export class GetGroupChildrenTask extends BaseGroupTask<Group[]> {
  get name(): string {
    return GetGroupChildrenTask.name;
  }
  ...
  async run(handler: DatabaseTransactionHandler): Promise<void> {
    ...
    this._result = await this.groupService.getGroupChildren(this.actor.id, group.id, handler);
    this.status = 'OK';
  }
}
