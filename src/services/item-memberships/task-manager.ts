...
export class TaskManager implements ItemMembershipTaskManager<Member> {
  ...
  createDeleteAllOfItemTask(member: Member, itemId: string): DeleteItemItemMembershipsTask {
    return new DeleteItemItemMembershipsTask(member, itemId, this.itemService, this.itemMembershipService);
  }
}
