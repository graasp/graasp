...
export class ExportDataRepository {
   ...
   */
  async getAppSettings(db: DBConnection, memberId: string): Promise<AppSetting[]> {
    if (!memberId) {
      throw new MemberIdentifierNotFound();
    }
   ...
   */
  async getByCreatorToExport(db: DBConnection, creatorId: CreatorId): Promise<ItemLike[]> {
