...
class FileService {
  ...
  async upload(member: Member, data: { file: Readable; filepath: string; mimetype?: string }) {
    ...
    try {
      ...
      void this.delete(member, filepath);
      this.logger.error(e);
  ...
  async setHeaders({
    ...
    if (replyUrl) {
      ...
      await reply.header('Access-Control-Allow-Credentials', 'true');
      await reply.redirect(StatusCodes.MOVED_TEMPORARILY, url);
    }
  }
}
