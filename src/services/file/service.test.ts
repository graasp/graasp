...
const s3Repository = fileRepositoryFactory(FileStorage.S3, { s3: MOCK_S3_CONFIG });
...
describe('FileService', () => {
  describe('constructor', () => {
    ...
    it('use local repository', () => {
      const repository = fileRepositoryFactory(FileStorage.Local, { local: MOCK_LOCAL_CONFIG });
      expect(repository).toBeInstanceOf(LocalFileRepository);
    });
    it('throws for conflicting settings', () => {
      expect(() => {
        fileRepositoryFactory(FileStorage.S3, { s3: MOCK_S3_CONFIG });
      }).toThrow(MalformedFileConfigError);

      expect(() => {
        fileRepositoryFactory(FileStorage.Local, { local: MOCK_LOCAL_CONFIG });
      }).toThrow(MalformedFileConfigError);
    });
  });
