...
export class ActionRepository {
   ...
   * @returns an array of filtered actions that need a permission generated by the member, start and end dates.
   */
  async getMemberActionsNeedPesrmission(
    memberId: string,
    ...
    return this.repository.find({
      where: {
        ...
        type: Not(In(actionTypesWithoutNeedOfPermission)),
      },
      order: { createdAt: 'DESC' },
      relations: {
        item: true,
   ...
   * @param filters.startDate date of actions after this date.
   * @param filters.endDate date of actions before this date.
   * @returns an array of filtered actions that don't require a permission generated by the member, start and end dates.
   */
  async getMemberActionsNoNeedForPesrmission(
    memberId: string,
    filters: { startDate: Date; endDate: Date },
  ): Promise<Action[]> {
    if (!memberId) {
    ...
    return this.repository.find({
      where: {
        member: { id: memberId },
        createdAt: Between(startDate, endDate),
        type: In(actionTypesWithoutNeedOfPermission),
      },
      order: { createdAt: 'DESC' },
      relations: {
        item: true,
