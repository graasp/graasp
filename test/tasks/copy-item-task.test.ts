...
const member = {id: 'memberId', name: 'memberName', type: MemberType.Individual, createdAt: 'createdAt', updatedAt: 'updatedAt', email:'email'} as Member;

const compareItemtoCopyItem = (idMapping: Map<string, string>, item: Partial<Item>, itemCopy: Item, parentItemId?: string) => {
  const {id: newId, name: newName, description: newDescription, extra: newExtra, creator: newCreator, path: newPath} = itemCopy;
  ...
  const parsedPath = path.split('.').map(id => idMapping.has(id) ? idMapping.get(id) : id).join('.');
...
describe('CopyItemTask', () => {
  ...
  const fakeItem1 = {id: 'fakeItem1', path:'fakeItem1', name:'name1', description: 'description1', extra: {some: 'extra1'}} as Item<SomeExtra>;
  const fakeItem2 = {id: 'fakeItem2', path: 'fakeItem1.fakeItem2' , name:'name2', description: 'description2', extra: {some: 'extra2'}} as Partial<Item<SomeExtra>>;
  const fakeItem3 = {id: 'fakeItem3', path: 'fakeItem1.fakeItem3', name:'name3', description: 'description3', extra: {some: 'extra3'}} as Partial<Item<SomeExtra>>;
  ...
  test('Should copy item and its descendants in parentItem with membership', async () => {
    ...
    itemService.create = jest.fn(async (data) => data as Item);
      ...
      for(const [i, subTask] of subTasks.entries()) {
        ...
        compareItemtoCopyItem(idMapping, itemTree[i], subTask.result as Item, parentItemId);
      }
